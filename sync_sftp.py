#!/usr/bin/env python3
"""
Script para sincronizar archivos locales con uno o más servidores SFTP.
Respeta el .gitignore y sincroniza en modo mirror (elimina archivos huérfanos).

Uso:
    python sync_sftp.py [--dry-run] [--no-delete] [--verbose] [--server NOMBRE]

Configuración:
    Crear archivo .env.sftp con:

    # Para un solo servidor (formato simple):
    ...
    # Para múltiples servidores:
        SFTP_SERVERS=produccion,desarrollo

        SFTP_HOST_produccion=10.0.0.1
        SFTP_PORT_produccion=22
        SFTP_USER_produccion=user1
        SFTP_PASS_produccion=pass1
        SFTP_REMOTE_PATH_produccion=/ruta/prod

        SFTP_HOST_desarrollo=10.0.0.2
        SFTP_PORT_desarrollo=22
        SFTP_USER_desarrollo=user2
        SFTP_PASS_desarrollo=pass2
        SFTP_REMOTE_PATH_desarrollo=/ruta/dev
"""
...
def get_server_configs(env_vars: dict) -> Dict[str, dict]:
    """
    Extrae configuraciones de servidor(es) del archivo .env.
    Soporta formato simple (un servidor) y múltiple (SFTP_SERVERS).
    """
    servers = {}

    # Verificar si hay múltiples servidores configurados
    if "SFTP_SERVERS" in env_vars and env_vars["SFTP_SERVERS"]:
        server_names = [s.strip() for s in env_vars["SFTP_SERVERS"].split(",") if s.strip()]

        for name in server_names:
            config = {
                "host": env_vars.get(f"SFTP_HOST_{name}"),
                "port": int(env_vars.get(f"SFTP_PORT_{name}", 22)),
                "user": env_vars.get(f"SFTP_USER_{name}"),
                "password": env_vars.get(f"SFTP_PASS_{name}"),
                "remote_path": env_vars.get(f"SFTP_REMOTE_PATH_{name}", "").rstrip("/"),
            }

            # Validar que tenga los campos requeridos
            missing = [k for k in ["host", "user", "password", "remote_path"]
                      if not config.get(k)]
            if missing:
                print(f"  [WARN] Servidor '{name}' tiene campos faltantes: {missing}")
                continue

            servers[name] = config
    else:
        # Formato simple (un solo servidor)
        if all(env_vars.get(k) for k in ["SFTP_HOST", "SFTP_USER", "SFTP_PASS", "SFTP_REMOTE_PATH"]):
            servers["default"] = {
                "host": env_vars["SFTP_HOST"],
                "port": int(env_vars.get("SFTP_PORT", 22)),
                "user": env_vars["SFTP_USER"],
                "password": env_vars["SFTP_PASS"],
                "remote_path": env_vars["SFTP_REMOTE_PATH"].rstrip("/"),
            }
...
def sync_to_server(
    ...
    """
    Sincroniza archivos locales a un servidor SFTP específico.
    Retorna dict con contadores: uploaded, updated, deleted.
    """
    host = config["host"]
    port = config["port"]
    user = config["user"]
    ...
    result = {"uploaded": 0, "updated": 0, "deleted": 0, "errors": 0}
    transport = None
    sftp = None

    try:
        print(f"\n  [1/3] Conectando a SFTP...")
        ...
        print(f"\n  [2/3] Escaneando archivos remotos...")
        ...
        to_delete = remote_files - local_files if not no_delete else set()

        print(f"\n  [3/3] Sincronizando...")
        print(f"        Nuevos: {len(to_upload)}")
        print(f"        Verificar: {len(to_update)}")
        ...
        if to_upload:
            ...
            for rel_path in sorted(to_upload):
                ...
                if dry_run:
                    ...
                    if sync_file(sftp, local_path, remote_path, verbose):
                        print(f"      [OK] {rel_path}")
                        result["uploaded"] += 1
        ...
        print(f"\n  Resumen {server_name}:")
        if dry_run:
            print(f"    (DRY-RUN) Subirían: {len(to_upload)}, Verificarían: {len(to_update)}, Eliminarían: {len(to_delete)}")
        else:
            print(f"    Subidos: {result['uploaded']}, Actualizados: {result['updated']}, Eliminados: {result['deleted']}")

    except paramiko.AuthenticationException:
        print(f"  [ERROR] Autenticación fallida para {server_name}")
        result["errors"] = 1
    except paramiko.SSHException as e:
        print(f"  [ERROR] Error SSH en {server_name}: {e}")
        result["errors"] = 1
    except Exception as e:
        print(f"  [ERROR] Error en {server_name}: {e}")
        result["errors"] = 1
    finally:
        if sftp:
            sftp.close()
...
def main():
    parser = argparse.ArgumentParser(
        description="Sincroniza archivos locales con uno o más servidores SFTP respetando .gitignore"
    )
    parser.add_argument(
        "--dry-run", "-n",
        action="store_true",
    ...
    parser.add_argument(
        "--server", "-s",
        type=str,
        help="Sincronizar solo a un servidor específico (por nombre)"
    )
    ...
    if not env_path.exists():
        ...
        print("\n# Para múltiples servidores:")
        print("  SFTP_SERVERS=prod,dev")
        print("  SFTP_HOST_prod=10.0.0.1")
        print("  SFTP_USER_prod=user1")
        print("  ...")
        sys.exit(1)

    env_vars = load_env_file(env_path)

    # Obtener configuraciones de servidores
    servers = get_server_configs(env_vars)

    if not servers:
        print(f"Error: No se encontraron servidores configurados en {env_path.name}")
        print("Configura SFTP_HOST/USER/PASS/REMOTE_PATH o usa SFTP_SERVERS para múltiples.")
        sys.exit(1)

    # Filtrar por servidor específico si se indicó
    if args.server:
        if args.server not in servers:
            print(f"Error: Servidor '{args.server}' no encontrado.")
            print(f"Servidores disponibles: {', '.join(servers.keys())}")
            sys.exit(1)
    ...
    print(f"Carpetas: {', '.join(INCLUDE_ONLY) if INCLUDE_ONLY else 'Todo'}")
    ...
    # Obtener archivos locales (solo una vez)
    ...
    # Sincronizar a cada servidor
    total_results = {"uploaded": 0, "updated": 0, "deleted": 0, "errors": 0}
    ...
    # Resumen final (solo si hay múltiples servidores)
    if len(servers) > 1:
        print("\n" + "=" * 60)
        print("RESUMEN TOTAL")
        print("=" * 60)
        if args.dry_run:
            ...
            print(f"  Subidos: {total_results['uploaded']}")
            print(f"  Actualizados: {total_results['updated']}")
            print(f"  Eliminados: {total_results['deleted']}")
            if total_results['errors'] > 0:
                print(f"  Servidores con error: {total_results['errors']}")
        print("=" * 60)
