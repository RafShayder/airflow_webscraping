...
def setup_logging(level: str = None) -> None:
    ...
    logging.basicConfig(
        level=getattr(logging, resolved_level, logging.INFO), handlers=[handler]
    )
        ...
        for conn_id_to_try in conn_ids_to_try:
            try:
                ...
                if isinstance(extras, dict):
                    ...
                    if (
                        "processed_destination" in extras
                        and "local_destination_dir" not in values
                    ):
                        values["local_destination_dir"] = extras[
                            "processed_destination"
                        ]

                    # Mapeos específicos para webindra (conexiones HTTP)
                    if conn.conn_type == "http" and (
                        "http_webindra" in conn_id_to_try
                        or "webindra" in conn_id_to_try
                    ):
                ...
                logger.debug(
                    f"Valores cargados desde Connection '{conn_id_to_try}': {list(values.keys())}"
                )

                # Si faltan credenciales en conexiones DAAS específicas, usar sftp_daas_{env}
                if conn_id in fallback_conn_ids and (
                    ...
                    for daas_conn_id in daas_conn_ids:
                        try:
                            ...
                            logger.debug(
                                f"Credenciales fallback desde '{daas_conn_id}' para '{conn_id_to_try}'"
                            )
                            break
                        except Exception:
                            continue
...
def _load_airflow_variables(
    prefix: str, section_name: str = None, env: str = None
) -> Dict[str, Any]:
    """
    ...
    try:
                    ...
                    if isinstance(value, str):
                        ...
                        if value_lower in ("true", "false"):
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"Error al parsear HEADERS como JSON desde '{var_name}': {e}. Usando valor como string."
                                )
                                values[field] = value
                        else:
                            values[field] = value
        ...
        logger.debug(
            f"Error al cargar Variables de Airflow con prefijo '{prefix}': {exc}"
        )
...
class LazyAirflowConfig(dict):
    ...
    def _load_section_from_airflow(self, section_name: str) -> dict:
        ...
        try:
            ...
            logger.debug(
                f"Lazy loading para sección '{section_name}' usando autodescubrimiento (conn_id='{conn_id}', var_prefix='{var_prefix}')"
            )
            ...
            if conn_values and isinstance(conn_values, dict):
                ...
                logger.debug(
                    f"Valores cargados desde Connection '{conn_id}_{self._env}' para sección '{section_name}': {list(conn_values.keys())}"
                )
            ...
            if var_values and isinstance(var_values, dict):
                ...
                logger.debug(
                    f"Valores cargados desde Variables '{var_prefix}*' para sección '{section_name}': {list(var_values.keys())}"
                )

            if section_config:
                logger.debug(
                    f"Sección '{section_name}' cargada exitosamente desde Airflow mediante autodescubrimiento"
                )
            else:
                logger.debug(
                    f"No se encontró configuración para sección '{section_name}' en Airflow"
                )
...
def _apply_airflow_overrides(config: dict, env: str) -> dict:
    ...
    for section_name in all_sections:
        ...
        if section_name not in merged_config:
            ...
            logger.debug(
                f"Sección '{section_name}' no existe en YAML, construyendo desde Airflow"
            )
        ...
        if isinstance(section_value, dict):
            ...
            logger.warning(
                f"Sección '{section_name}' no es un diccionario (tipo: {type(section_value)}), inicializando como diccionario vacío"
            )
            section_config = {}
        ...
        if mapping:
            ...
            var_prefix = f"{section_name.upper()}_"
            logger.debug(
                f"Sección '{section_name}' no está en mapeo, usando auto-descubrimiento (conn_id='{conn_id}', var_prefix='{var_prefix}')"
            )
        ...
        if conn_id:
            ...
            if conn_values and isinstance(conn_values, dict):
                ...
                section_config.update(conn_values)
                logger.debug(
                    f"Overrides desde Connection '{conn_id}' (env={env}) aplicados a sección '{section_name}'"
                )
            elif conn_values:
                logger.warning(
                    f"Connection '{conn_id}' devolvió un valor no-diccionario (tipo: {type(conn_values)}), ignorando"
                )
        ...
        var_values = _load_airflow_variables(
            var_prefix, section_name if mapping else None, env
        )
        ...
        if var_values and isinstance(var_values, dict):
            ...
            logger.warning(
                f"Variables para '{section_name}' devolvieron un valor no-diccionario (tipo: {type(var_values)}), ignorando"
            )
...
def load_config(env: str | None = None) -> dict:
    ...
    try:
        ...
        if dotenv_path.exists():
            ...
            logger.debug(
                "No se encontró archivo .env en energiafacilities, usando variables del sistema"
            )
        ...
        if env:
            ...
            if airflow_env:
                ...
                logger.debug(
                    f"Entorno obtenido desde Airflow Variable ENV_MODE: {resolved_env}"
                )
            else:
                resolved_env = os.getenv("ENV_MODE", "dev").lower()
                logger.debug(
                    f"Entorno obtenido desde variable de entorno ENV_MODE: {resolved_env}"
                )
        ...
        if not os.path.exists(config_path):
            if _is_airflow_available():
                ...
                logger.debug(
                    "Construyendo configuración completamente desde Airflow Variables/Connections"
                )
                yaml_config = {}  # Config vacío, se construirá desde Airflow
            else:
                logger.error(f"No existe el archivo de configuración: {config_path}")
                raise FileNotFoundError(
                    f"No existe el archivo de configuración: {config_path} y Airflow no está disponible"
                )
        else:
        ...
        logger.debug(
            f"Configuración cargada para entorno '{resolved_env}'. "
