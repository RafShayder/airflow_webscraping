    ...
    for bd_col, excel_expected in column_mapping.items():
        ...
        if excel_expected in excel_columns:
            ...
            if expected_normalized in excel_cols_normalized:
                ...
                logger.info(f"Mapeo flexible encontrado: {bd_col} -> '{excel_found}' (esperado: '{excel_expected}')")
            else:
                # 3. No se encontró, se insertará como NULL
                logger.warning(f"No se encontró columna para '{bd_col}' (esperado: '{excel_expected}'). Se insertará como NULL.")
