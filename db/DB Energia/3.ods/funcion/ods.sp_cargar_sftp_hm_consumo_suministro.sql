CREATE OR REPLACE PROCEDURE ods.sp_cargar_sftp_hm_consumo_suministro()
 LANGUAGE plpgsql
AS $procedure$
DECLARE
  v_inicio        timestamp(0) := clock_timestamp()::timestamp(0);
  v_id_sp         integer      := 'ods.sp_cargar_sftp_hm_consumo_suministro()'::regprocedure::oid::int;
  v_sp_name       text         := 'ods.sp_cargar_sftp_hm_consumo_suministro()'::regprocedure::text;
  v_inserted      integer      := 0;
  v_err_invalid   integer      := 0;
  v_err_dups_pair integer      := 0;  -- duplicados por (num_recibo, per_consumo)
  v_err_nulos_nr  integer      := 0;  -- (NUEVO) registros con num_recibo nulo/vacío/'nan'/'null'
...
BEGIN
  ...
  WITH invalid AS (
    ...
    FROM raw.sftp_mm_consumo_suministro r
    WHERE r.num_recibo IS NOT NULL
      AND btrim(r.num_recibo) <> ''
      -- sin dígitos tras un guion (-,–,—) opcionalmente con espacios
  ...
  SELECT
    r.num_recibo, r.cod_suministro, r.tarifa, r.pot_contr, r.titularidad, r.distribuidor,
    ...
    clock_timestamp()::timestamp(0), 'raw.sftp_mm_consumo_suministro'
  FROM cand_invalid r
  LEFT JOIN public.error_energia e
        ...
        AND e.tabla_origen = 'raw.sftp_mm_consumo_suministro'
  ...
  DELETE FROM raw.sftp_mm_consumo_suministro r
  USING invalid i
  WHERE r.ctid = i.rid;

  /* ========= 1.b) (NUEVO) NULOS / VACÍOS en num_recibo =========
     Si quieres SOLO NULL estricto, reemplaza la condición del WHERE por:
  ...
  */
  ...
  LEFT JOIN public.error_energia e
        ...
        AND e.tabla_origen = 'raw.sftp_mm_consumo_suministro'
        AND e.fecha_carga::date = clock_timestamp()::date
  WHERE e.fecha_carga IS NULL;  -- clave para no duplicar en el mismo día
  ...
  -- (NUEVO) Total de errores del día
  ...
  WITH base AS (
    SELECT
      ...
      -- num_recibo final (texto limpio y truncado)
      CASE
      ...
      public.try_numeric(r.pot_contr)::numeric(18,2) AS pot_contr,
  ...
  dedup AS (
    -- ODS PK = num_recibo. Conservamos la primera aparición por num_recibo.
  ...
  v_estado := 'DONE';
  v_msj    := format(
    'Insertados en ODS: %s | Enviados NUEVOS a public.error_energia (hoy): %s (inválidos: %s, duplicados num_recibo+per_consumo: %s, num_recibo nulo/vacío: %s) | Reglas: inválidos (sin dígitos tras guion), duplicados exactos por par y num_recibo nulo/vacío.',
